<template>
  <BaseReusableSwiper class="xl:!hidden" :options="{ spaceBetween: 40 }">
    <SwiperSlide v-for="(subcategory, ix) in categoriesDesktop" :key="ix">
      <div
        class="grid grid-cols-4 w-fit gap-x-[2.4rem] gap-y-[3rem] grid-rows-2 lg:hidden"
      >
        <BaseCategoryCard
          v-for="(category, ix) in subcategory"
          :key="ix"
          v-bind="category"
        />
      </div>
  ...
  <BaseReusableSwiper
    class="!hidden md:!flex sm:!hidden"
  ...
  <BaseReusableSwiper
    ...
    :options="{ slidesPerView: '1.5', spaceBetween: 20 }"
  >
    <SwiperSlide v-for="(category, ix) in categories.slice(0, 12)" :key="ix">
...
function splitArrayBy<T>(array: T[], number: number): T[][] {
  ...
  for (let i = 0; i < Math.ceil(categories.length / number); i++) {
    splitCategories = [
      ...splitCategories,
      categories.slice(i * number, i * number + number),
    ];
  }
